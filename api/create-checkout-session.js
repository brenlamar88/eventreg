...
export default async function handler(req, res) {
  ...
  try {
    const { email, lineItems, party, eventId } = req.body || {};
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: `${process.env.PUBLIC_BASE_URL}/?status=success&session_id={CHECKOUT_SESSION_ID}`,
      cancel_url: `${process.env.PUBLIC_BASE_URL}/?status=cancelled`,
      metadata: { eventId: eventId || "boil85", party: String(party || 1) },
    });
