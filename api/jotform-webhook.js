...
// Parse Jotform's URL-encoded body manually — Vercel doesn't auto-parse it.
...
// Jotform field names vary by form builder. This tries multiple common patterns.
function getField(fields, ...keys) {
  for (const key of keys) {
    if (fields[key] !== undefined && fields[key] !== "") return fields[key];
    // Also check bracket sub-fields: q3_firstName[first]
...
// Combine first + last name sub-fields (q3_fullName[first] / [last])
function getFullName(fields) {
  ...
  // Build from parts
  const first =
    fields["q3_firstName[first]"] ||
    fields["q4_firstName[first]"] ||
    getField(fields, "q3_firstName", "q4_firstName") ||
    "";
  const last =
    fields["q3_firstName[last]"] ||
    fields["q4_firstName[last]"] ||
    getField(fields, "q3_lastName", "q4_lastName", "q5_lastName") ||
    "";
...
function getPhone(fields) {
  // Jotform phone widget: q6_phoneNumber[area] + [phone], or q6_phoneNumber[full]
  for (const fk of Object.keys(fields)) {
  ...
  // Fallback: any field with "phone" in name
  return getField(fields, "q6_phone", "q7_phone", "q5_phone", "q8_phone");
}

function getPartySize(fields) {
  const raw =
    getField(fields, "q7_partySize", "q6_partySize", "q8_partySize", "q5_partySize",
      "q7_party", "q6_party", "q9_partySize", "q10_partySize") ||
    getField(fields, "q7_howMany", "q6_howMany");
...
export default async function handler(req, res) {
  ...
  if (WEBHOOK_SECRET && req.headers["x-webhook-secret"] !== WEBHOOK_SECRET) {
    console.warn("jotform-webhook: bad secret");
  ...
  // Map fields to registrant row
  const name = getFullName(fields);
  const email =
    getField(fields, "q5_email", "q4_email", "q6_email", "q3_email", "q7_email", "q8_email");
  const phone = getPhone(fields);
  const ranch =
    getField(fields, "q8_ranch", "q9_ranch", "q6_ranch", "q7_ranch", "q10_ranch",
      "q8_business", "q9_business", "q8_organization", "q9_organization");
  const party = getPartySize(fields);
  const sponsorName =
    getField(fields, "q9_sponsor", "q10_sponsor", "q11_sponsor", "q12_sponsor",
      "q9_sponsorName", "q10_sponsorName");
  const submissionId = fields["submissionID"] || fields["submission_id"] || null;

  if (!name && !email) {
    console.warn("jotform-webhook: no name or email found in payload");
    // Still return 200 so Jotform doesn't keep retrying a bad form config
    return res.status(200).json({ ok: false, reason: "no_name_or_email" });
  }

  const amount = party * 100; // $100/person — adjust if needed
  ...
  const row = {
    ...
    party,
    amount,
    status: "pending", // organizer can update to paid after payment
    source: "jotform",
    checked_in: false,
  ...
  try {
    // Check for duplicate by submissionId (notes field) or email+event
    if (submissionId) {
      const checkRes = await fetch(
        `${SB_URL}/rest/v1/${TABLE}?event_id=eq.${EVENT_ID}&notes=eq.jotform:${encodeURIComponent(submissionId)}&select=id`,
        { headers: sbHeaders }
      );
      const existing = await checkRes.json();
      if (Array.isArray(existing) && existing.length > 0) {
        console.log("jotform-webhook: duplicate submission, skipping:", submissionId);
        return res.status(200).json({ ok: true, duplicate: true });
      }
    }

    const insertRes = await fetch(`${SB_URL}/rest/v1/${TABLE}`, {
      method: "POST",
      headers: sbHeaders,
    ...
    console.log("jotform-webhook: inserted registrant:", result?.[0]?.id);
