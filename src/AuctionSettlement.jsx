...
export default function AuctionSettlement() {
  ...
  const findPhone  = (name) => (people.find((p) => p.name.toLowerCase() === name.toLowerCase()) || {}).phone    || "";
  // Buyer email + phone under their name — so a consignor can reach the buyer
  // to arrange delivery. Only renders when we have contact info on file.
  const buyerContact = (name) => { const em = findEmail(name), ph = findPhone(name); return (em || ph) ? <div style={{ fontSize: 11, color: "var(--inkSoft)", marginTop: 2, lineHeight: 1.35 }}>{[ph, em].filter(Boolean).join(" · ")}</div> : null; };
  ...
  // Strip the "#405 - " bidder prefix for BOTH consignor and buyer when a name
  // is picked from the datalist (free-typed names pass through unchanged).
  const onNameChange = (which, v) => { const name = parseBuyerInput(v); setForm((p) => ({ ...p, [which + "Name"]: name, [which + "Ranch"]: findRanch(name) || p[which + "Ranch"] })); };
  ...
  const connect = async () => {
    ...
    try {
      ...
      try { const rr = await fetch(withEvent("/api/registrants"), { headers: hdr() }); if (rr.ok) { const rows = await rr.json(); const seen = new Set(); const ppl = []; rows.forEach((x) => { const k = (x.name || "").toLowerCase(); if (x.name && !seen.has(k)) { seen.add(k); ppl.push({ name: x.name, ranch: x.ranch || "", bidderNo: x.bidder_number || "", email: x.email || "", phone: x.phone || "" }); } }); setPeople(ppl); } } catch {}
